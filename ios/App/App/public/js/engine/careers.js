...
(function () {
  ...
  function fillVacancy(gameState, school, rng, depth) {
    ...
    if (depth < 2 && school.prestige >= 45 && rng.bool(0.6)) {
      const targets = Object.values(gameState.world.schools)
        .filter((s) => {
          ...
          if (Legacy.hasLeftSchool(school, c.id)) return false; // never re-hire a coach who left (Update 19)
    ...
    if (depth < 2 && rng.bool(0.55)) {
      ...
      Object.values(gameState.world.schools).forEach((s) => {
        ...
        if (Legacy.hasLeftSchool(school, c.id)) return; // a program that lost them won't promote them back (Update 19)
    ...
    //    never from the program that just let them go (Update 19). A "second
  ...
  function runAssistantCarousel(gameState, rng) {
      ...
      .forEach((school) => {
        ...
        const cands = Object.values(gameState.world.schools)
          ...
          .filter(({ c }) => !c.isPlayer && (c.reputation || 0) >= school.prestige * 0.4 &&
            !Legacy.hasLeftSchool(school, c.id)) // no returns to a program they left (Update 19)
    ...
    // Trim dead ids out of the per-program former-staff ledgers (Update 19).
